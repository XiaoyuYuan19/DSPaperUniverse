...
def load_data():
    ...
    with open('../network_data/keywords_set.pkl', 'rb') as keywords_set_file:
        keywords_set = pickle.load(keywords_set_file)

    with open('../network_data/keyword_info_map.pkl', 'rb') as keyword_info_map_file:
        keyword_info_map = pickle.load(keyword_info_map_file)

    coOccurrence_matrix = np.load('../network_data/cooccurrence_matrix.npy')
...
def visualize_keyword_network(nodes, edges):
    graph = (
        Graph()
        .add(
            "",
            nodes,
            edges,
            repulsion=50,  # You can adjust this value to control node spacing
            layout="force",
            label_opts=opts.LabelOpts(is_show=False),  # Hide node labels
        )
        .set_global_opts(
            title_opts=opts.TitleOpts(title="Paper Co-Occurrence Network"),
        )
    )
...
def network_visu2(keywords_set, keyword_info_map, coOccurrence_matrix):
    ...
    visualize_keyword_network(nodes, edges)